...
- 在 `top.responseTimeEndParameters` 末端加入 `{ UUID: uuid, requestTimeField: dateJs.getCurrentTimeStamp() }`。

### 副作用

- 建立或改寫最上層頁面的 `responseTimeEndParameters` 陣列；後續經包裝的一般表單送出會讀取並清空該集合。
